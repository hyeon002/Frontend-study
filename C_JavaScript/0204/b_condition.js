...
// >> if문과 else 문 사이에 위치, 여러 개의 조건을 확인할 수 있게 해주는 제어 구조
...
if (조건1) {
  ...
  조건1과 조건2이 거짓이고, 조건3가 참일 때 실행되는 코드
  ...
} else {
...
*/
// case 의 값의 경우 조건식의 데이터와 같은 데이터타입을 가져야 함
// break문 (필수 X)
...
// 중첩 else if 문과 삼항 연산자
// : JS의 if 조건문은 해당 조건들 내에서 또 다른 조건식을 작성할 수 있다.
/*
if (조건1) {
  조건1이 참일 때 실행될 코드
  if (조건2) {
...
// 위의 중첩 if문을 삼항연산자로 변환
let grade = 
score>100 || score<0 ? 'incollect score' 
: score>=90 ? 'A' 
: score>=80 ? 'B'
: score>=70 ? 'C' : 'F';

console.log(`${grade} 입니다!~!~!`);