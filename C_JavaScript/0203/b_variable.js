...
! 변수 (variable)
  ...
  ? 1. 변수 선언
    : 변수를 사용하기 위해 변수를 다른 변수들과 구별하는 이름을 붙여주는 것
    >> 값을 저장할 컴퓨터의 메모리 공간에 이름을 붙이는 것 

  ? 2. 변수명 명명규칙
    - 첫 글자 : 영문자, 언더스코어(_), 달러($)만 사용 가능 (이후 숫자 사용 가능)
    - 띄어쓰기 허용 X
    - 기호는 언더스코어(_), 달러($)만 사용 가능
    - 영어 대소문자 구별  ex) now와 Now는 다른 변수
    - 예약어 사용 X
...
var x = 5;
console.log(x);   // 5
var x = 6;        // 동일한 이름으로 재선언
console.log(x);

// ? let
// let y;
// >> 해당 생략된 변수는 TDZ (일시적 사각지대, Temporal Dead Zone)
// let과 const로 선언된 변수는 선언문에 도달하고 초기화될 때까지 TDZ 에 존재 

// console.log(y); - Error
// Uncaught ReferenceError: Cannot access 'y' before initialization at b_variable.js:86:13
let y = 3;        // 변수 선언 및 초기화
console.log(y);
// let y = 4; - Error
...
? 상수 명명규칙
  - 변수의 명명규칙과 동일하지만, UPPER_SNAKE_CASE 를 사용
    : 모든 영문자를 대문자로 작성
...
// const MY_CONSTNAT = '반갑습니다'; - Error (재선언 불가)
// const MY_CONSTNAT = '반갑습니다'; - Error ??